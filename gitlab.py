...
DEFAULT_SETTINGS = {
    'debug_mode': False,
    'gitlab_hostnames': ['gitlab.com']
}

plugin_dir = os.path.abspath(os.path.dirname(__file__))
settings = {}
debug_mode = DEFAULT_SETTINGS['debug_mode']
gitlab_hostnames = DEFAULT_SETTINGS['gitlab_hostnames']

def plugin_loaded():
    global settings, debug_mode, gitlab_hostnames
    settings = sublime.load_settings('Gitlab Tools.sublime-settings')
    if settings.get('debug_mode'):
        debug_mode = settings.get('debug_mode')
    if settings.get('gitlab_hostnames'):
        gitlab_hostnames = settings.get('gitlab_hostnames')

class NotAGitRepositoryError(Exception):
    pass


class NotAGitlabRepositoryError(Exception):
    pass
...
class GitRepo(object):
    def __init__(self, path):
        ...
        if not self.info:
            raise NotAGitlabRepositoryError("Failed to read url of remote '%s'" % remote_alias)
    ...
    def parse_remote(self, remote_alias, remote):
        for hostname in gitlab_hostnames:
...
class GitlabWindowCommand(sublime_plugin.WindowCommand):
    def rootdir(self):
        if self.filename():
...
def with_repo(func):
    ...
    def wrapper(self):
        ...
        except (NotAGitRepositoryError, NotAGitlabRepositoryError) as err:
            sublime.message_dialog("Gitlab repository not found: %s" % err)
